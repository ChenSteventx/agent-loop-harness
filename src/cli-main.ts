...
import { CommandRunner, WorktreeService } from "./execution.js";
...
program
  ...
  .action(async () => runFastCli(process.argv.slice(2)));
...
async function runReplayCommand(options: {
  ...
  await withEvaluationStoresAsync(async (development, evaluationStore, home) => {
    ...
    const evaluator = new VerifyOnlyEvaluator(commandRunnerFor(resolveProjectAdapter()));
...
function createEvaluationFullTaskExecutor(memoryStore?: EvaluationStore): ReturnType<typeof createFullTaskExecutor> {
  ...
  return createFullTaskExecutor({
    ...
    commandRunner: commandRunnerFor(projectAdapter),
...
function resolveProjectAdapter(): ProjectAdapter {
  if (!resolvedProjectAdapter) {
    ...
    resolvedProjectAdapter = configPath
      ...
      : new GenericNodeProjectAdapter("generic-node/v2", process.env.AGENT_LOOP_OCI_IMAGE ?? null);
  }
  return resolvedProjectAdapter;
}

function commandRunnerFor(projectAdapter: ProjectAdapter): CommandRunner {
  return new CommandRunner({ imageDigest: projectAdapter.verificationImageDigest ?? undefined });
}
